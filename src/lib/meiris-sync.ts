 ...
 *
 * 対象は users.ad_opt_in = 1 かつ meiris_synced_at IS NULL の会員のみ。
 * 未同意 (ad_opt_in = 0) の会員は絶対に対象にしない。
 *
  ...
  return db
    .prepare(
      `SELECT
         ...
         SUM(CASE WHEN ad_opt_in = 1 AND meiris_synced_at IS NOT NULL THEN 1 ELSE 0 END) AS already_synced,
         SUM(CASE WHEN ad_opt_in = 1 AND meiris_synced_at IS NULL THEN 1 ELSE 0 END) AS pending
       FROM users`
    )
 ...
 */
export function selectTargets(limit: number, resend = false, sinceDays?: number): { id: number; email: string }[] {
  const conds = ['ad_opt_in = 1'];
  ...
  return db
    ...
    .all(limit) as { id: number; email: string }[];
}
...
export async function flushPending(
  ...
  for (let i = 0; i < targets.length; i += MEIRIS_MAX_BATCH) {
    ...
    const outcome = await sendContacts(chunk.map((t) => t.email));
    ...
    const tx = db.transaction((rows: { id: number; email: string }[]) => {
