...
export function normalizeShopName(raw) {
  ...
  n = n.replace(/[・･\-―—‐ーｰ~～〜!！?？.,，、。'"`’‘“”]/g, '');
  // 3. 括弧と中身を削除
  //    ただし店名全体が括弧で囲われていると全部消えて空文字になる。
  //    空文字同士は「一致」してしまうので、無関係な店が同一店として統合されかねない。
  //    (2026-09-06 実測: 「熟アロマ いそじ」「むちむちお姉さん」【艶〜ＥＮ〜】など active 8店が該当)
  //    → 消えてしまう場合は括弧の記号だけ落として中身は残す。
  {
    const stripped = n.replace(/[（(\[【〔『「].*?[）)\]】〕』」]/g, '');
    n = stripped.trim() === '' ? n.replace(/[（()\[\]【】〔〕『』「」]/g, '') : stripped;
  }
  // 4. スペース全削除
  n = n.replace(/[\s　]/g, '');
